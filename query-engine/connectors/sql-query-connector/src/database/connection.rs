...
use connector_interface::{
    filter::Filter, Connection, QueryArguments, ReadOperations, RecordFilter, Transaction, WriteArgs, WriteOperations,
};
...
{
    fn start_transaction<'a>(&'a self) -> BoxFuture<'a, Result<Box<dyn Transaction<'a> + 'a>, SqlError>> {
        let fut_tx = self.inner.start_transaction();
        let connection_info = self.connection_info;

        let b = self.catch(async move {
