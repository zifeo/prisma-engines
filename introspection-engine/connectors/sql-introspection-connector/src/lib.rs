...
pub type SqlError = anyhow::Error;

use datamodel::{parse_configuration, parse_datamodel, Datamodel};
// pub use error::*;
...
pub async fn introspect<'a>(
    ...
    backend: Box<dyn SqlSchemaDescriberBackend + 'a>,
) -> anyhow::Result<String> {
    // let config = parse_configuration(datamodel)
    //     .map_err(|err| err.to_pretty_string("meh.prisma", datamodel))
    //     .map_err(|message| anyhow::Error::msg(message))?;

    let previous_datamodel = parse_datamodel(datamodel)
        .map_err(|err| err.to_pretty_string("meh.prisma", datamodel))
        .map_err(|message| anyhow::Error::msg(message))?;

    let sql_schema = backend.describe(schema_name).await?;

    let introspection_result =
        calculate_datamodel::calculate_datamodel(&sql_schema, &sql_family, &previous_datamodel.subject)?;

    let introspected = datamodel::render_datamodel_to_string(&introspection_result.data_model);
