    ...
    fn column_type_change(&self, differ: &ColumnDiffer<'_>) -> Option<ColumnTypeChange> {
        ...
        if !native_types_enabled {
            ...
            dbg!(match dbg!((previous_type, next_type)) {
                ...
                (Some(previous), Some(next)) => native_type_change_riskyness(previous, next, self.is_cockroachdb()),
...
fn native_type_change_riskyness(
    ...
    let cast = || {
        Some(match previous {
            ...
            Integer => match next {
                PostgresType::SmallInt | PostgresType::BigInt if is_cockroachdb => return None,
                SmallInt => RiskyCast,
                BigInt => SafeCast,
                Decimal(params) => match params {
            ...
            BigInt => match next {
                PostgresType::SmallInt | PostgresType::Integer if is_cockroachdb => return None,
                SmallInt => RiskyCast,
                Integer => RiskyCast,
                Decimal(params) => match params {
